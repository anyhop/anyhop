...
providers than anyhop does.

The two projects solve different halves of the problem. gluetun gives one
container **one** VPN tunnel and makes everything that joins it exit there. anyhop
...
**anyhop** — many tunnels in one process. Each channel is a WireGuard peer exposed
...
Sibling containers point at anyhop's router entrypoint and stop caring: each
...
one of anyhop's two, the example above cannot run at all — which is exactly why
the next section exists.

## Choose gluetun when

- Your provider is one of the 23 it supports and not one of anyhop's two. This is
...
- You need OpenVPN today, or a provider that only offers WireGuard through
  gluetun's custom-provider path. (anyhop's OpenVPN support is still only
  [planned](vpn-provider-research.md): sing-box 1.14 provides the OpenVPN
  client, but anyhop has not built the provider path on top of it yet.)
- One exit is all you want, and "every container in this stack goes through
  Sweden" is the whole requirement.
- You want provider-side port forwarding for a torrent client — anyhop has no
  equivalent.
