...
``http.server``, JSON that is a 1:1 projection of ``anyhop.service`` (no
...
* Bound to ``127.0.0.1`` unless the operator opts into a network bind with
  ``ANYHOP_API_LISTEN`` (the container/compose knob; invalid values fall back to
  loopback, never widen). The secret can be injected via ``ANYHOP_API_SECRET`` /
  ``ANYHOP_API_SECRET_FILE`` so compose siblings can be handed the same value;
...
* Browsers use a per-installation ``anyhop-<rand>.localhost`` hostname (cookies
...
def _mint_host() -> str:
    """A per-installation browser hostname, e.g. ``anyhop-3f9c2ab1.localhost``.
    ...
    sent to every other local web app on any port — and two anyhop homes would
    collide. A random name no other service occupies isolates the session.
    """
    return f"anyhop-{secrets.token_hex(4)}.localhost"
...
def _valid_control_api(cfg) -> dict | None:
    ...
    if re.fullmatch(r"[0-9a-f]{64}", secret) is None:
        return None
    if re.fullmatch(r"anyhop-[0-9a-f]{8}\.localhost", host) is None:
        return None
...
def control_api() -> dict:
    ...
    Locking and durable publishing live in :func:`anyhop.fsio.generated_endpoint`
...
class ApiConfigError(Exception):
    """An unusable ``ANYHOP_API_*`` configuration. The server refuses to start
...
def _listen_config(api: dict) -> dict:
    ...
    for a wildcard bind), ``net`` True when ``ANYHOP_API_LISTEN`` names a
    ...
    raw = (os.environ.get("ANYHOP_API_LISTEN") or "").strip()
    if not raw:
        return contract
    parsed = _parse_listen(raw)
    if parsed is None:
        applog.log(
            f"api: invalid ANYHOP_API_LISTEN {raw!r} — keeping the loopback "
...
def _api_secret(api: dict) -> str:
    """The effective Bearer/signing secret: injected via ``ANYHOP_API_SECRET``
    or ``ANYHOP_API_SECRET_FILE`` (exactly one), else the minted per-install one.
    ...
    """
    env = os.environ.get("ANYHOP_API_SECRET")
    path = os.environ.get("ANYHOP_API_SECRET_FILE")
    if env is not None and path is not None:
        raise ApiConfigError(
            "both ANYHOP_API_SECRET and ANYHOP_API_SECRET_FILE are set — set exactly one"
        )
    if path is not None:
        try:
            ...
            raise ApiConfigError(
                f"ANYHOP_API_SECRET_FILE {path!r} is unreadable: {e}"
            ) from e
...
def effective_api() -> dict:
    ...
    "host", "net"}`` — what same-machine tools (``anyhop ui``, the tray) use.
...
def wait_until_serving(timeout: float = 6.0) -> bool:
    ...
    process (after the real bind failed), and ``anyhop ui`` must never hand its
...
def mint_login_url() -> str:
    """A one-time login URL for ``anyhop ui`` — carries a single-use token.
    ...
    if not _health_ok(api):
        raise ApiConfigError(
            f"no anyhop daemon owns the control endpoint at {api['address']}"
        )
...
def _asset(name: str) -> bytes | None:
    """Read a bundled static asset from ``anyhop/assets`` (shipped in the wheel),
    or None if it isn't one we ship."""
    if "/" in name or "\\" in name or ".." in name:
        return None  # no path traversal — flat asset dir only
    try:
        return (resources.files("anyhop") / "assets" / name).read_bytes()
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def version_string(self) -> str:
        ...
        # Server header names anyhop only, not the interpreter revision.
    ...
    def _get(self):
        ...
        if path == "/health":
            # unauthenticated on purpose: `anyhop ui` uses it to prove the
            # process behind the contract port is really anyhop *before* handing
    ...
    def _root(self, query: dict):
        ...
        # `anyhop ui` lands here with a one-time token: consume it, set the session
        ...
        if body is None:  # assets missing/misplaced — never a silent blank page
            return self._send(
                500,
                b"anyhop Web UI assets are missing from this build. "
                b"Try `anyhop restart`, or reinstall anyhop.",
    ...
    # -- /api/v1 (a 1:1 projection of anyhop.service; no business logic here) --
    def _call(self, fn, *args, **kwargs):
        ...
        from anyhop.state import RevisionConflict
...
def _channel_query(path: str) -> str | None:
    """The optional ``?channel=`` filter (same ref grammar as ``anyhop test``)."""
...
def build_server() -> ThreadingHTTPServer:
    ...
    (``ANYHOP_API_LISTEN``, ``ANYHOP_API_SECRET[_FILE]``). Wires the handler's
...
def start_in_thread() -> str | None:
    ...
    try:
        ...
        return None
    Thread(target=httpd.serve_forever, name="anyhop-api", daemon=True).start()
    url = ui_url()
    if lc["net"]:
        applog.log(
