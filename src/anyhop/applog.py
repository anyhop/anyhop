"""anyhop's own operation log: ``~/.anyhop/anyhop.log``.

This is *anyhop's* record of what it did — providers/channels added or removed,
...
the CLI and the applier daemon append here, so a user running ``anyhop logs -f``
...
# written to stderr so `docker logs` sees the same timeline as `anyhop logs`.
...
def log(message: str) -> None:
    ...
    file is replayed raw into terminals by ``anyhop logs``/``logs -f`` — a
    hostile provider/user label must not clear the screen there, and an
    embedded newline must not forge extra log lines.
    """
    from anyhop import output  # function-level: applog is imported by everything
...
def follow(n: int = 50) -> None:
    """Print the last ``n`` lines then stream new ones (``anyhop logs -f``).
