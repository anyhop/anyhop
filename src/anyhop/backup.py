...
disabled). The key is deliberately outside :func:`anyhop.state.config_signature`,
...
*is* anyhop's runtime, and anyhop installs no timers of its own. A backup pass is
...
byte. Retention prunes only ``anyhop-backup-*.yaml`` files, never anything else.
...
from anyhop import applog, bundle, fsio, paths
from anyhop.state import Store

DEFAULT_EVERY_HOURS = 24.0
DEFAULT_KEEP = 7
_PREFIX = "anyhop-backup-"
_SUFFIX = ".yaml"
...
def backup_files(directory: Path) -> list[Path]:
    """This rotation's backup files, newest first. Only ``anyhop-backup-*.yaml``
