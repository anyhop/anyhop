...
declarative: ``anyhop export`` is a convenience that emits the declarative form
...
rejects a token provider without one, since anyhop needs it to resolve servers
...
def dumps(data: dict) -> str:
    ...
    lines = [
        "# anyhop setup bundle — apply with `anyhop import <file>` (merge into the",
        "# current setup) or `anyhop import <file> --replace (overwrite it).",
...
def loads(text: str) -> dict:
    ...
    if not isinstance(data, dict):
        raise BundleError([("bundle", "not an anyhop bundle (root is not a mapping)")])
    return data
...
def _header_errors(data: dict, errors: list) -> None:
    if data.get("kind") != BUNDLE_KIND:
        errors.append(("kind", f"not an anyhop bundle (expected kind: {BUNDLE_KIND})"))
    ...
    if not isinstance(version, int) or isinstance(version, bool) or version < 1:
        ...
        errors.append(
            (
                "bundle_version",
                f"bundle version {version} is newer than this anyhop understands "
                f"(max {BUNDLE_VERSION}) — upgrade anyhop",
            )
        )
...
def _validate_port(value, path: str, errors: list) -> int:
    """An optional explicit port declaration: absent/0 means "let anyhop
...
def _resolve_token_wg(
    ...
    **Disabled channels are exempt from fresh resolution** — the provider API
    ...
    later ``anyhop channels enable`` resolves it then.
