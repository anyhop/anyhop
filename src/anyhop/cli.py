"""anyhop command-line interface.

The data model is provider-centric: **each provider owns a list of channels**.
You add a provider once (``anyhop providers add <name>``), then add channels
under it (``anyhop channels add <name> --country …``). All of it lives in one
...
def _legacy_channel_provider(args) -> str | None:
    ...
    if len(args.refs) != 1:
        raise service.ServiceError(
            ...
            "anyhop channels rm <provider> --channel <name>"
        )
...
def cmd_routes_lan(args):
    ...
    if router["lan_direct"]:
        ...
        print(
            ...
            "this protection on). Re-enable:  anyhop routes lan on"
        )
    if args.verbose:
...
def cmd_routes_geo(args):
    if args.action == "ls":
        ...
        if not result["manifest_populated"]:
            ...
            print(
                "No category list cached yet — run `anyhop routes geo refresh` "
...
def _print_geo_status(result: dict) -> None:
    ...
    for gkind, info in sorted(result["kinds"].items()):
        ...
        if missing:
            ...
            print("    fetch with: anyhop routes geo refresh")
...
def cmd_tun(args):
    ...
    if args.trial is not None:
        if args.state != "on":
            raise service.ServiceError("--trial only applies to: anyhop tun on")
        result = service.tun_trial_arm(args.trial)
        print(
            f"TUN mode ON — trial window: {result['trial']['seconds']}s.\n"
            "  Keep it:  anyhop tun confirm\n"
    ...
    if router["tun"]:
        ...
        print(
            ...
            "Enable:  anyhop tun on  (needs root)"
        )
...
def _test_strict_exit(args, result: dict) -> None:
    ...
    per-channel counterpart of `anyhop health` (daemon liveness), for monitoring.
...
def _run_streaming_test(args):
    ...
    if not stream:
        ...
        _test_strict_exit(args, result)
        return
    # No trailing summary line: plain `anyhop test` prints none either, and a
...
def _print_bundle_summary(result: dict) -> None:
    ...
    if created:
        print(
            ...
            "ports shown by: anyhop status"
        )


def cmd_export(args):
    result = service.setup_export()
    out = args.out or f"anyhop-backup-{time.strftime('%Y%m%d-%H%M%S')}.yaml"
    what = (
...
def cmd_backup(args):
    ...
    if b["enabled"]:
        ...
        print("  Backups run while the anyhop daemon is running.")
        if first or changing:
            _print_backup_secret_note(b)
    else:
        print("Scheduled backups off — enable: anyhop backup on")
        if b["count"]:
...
def cmd_start(args):
    ...
    if result["has_channels"]:
        ...
        print(
            "Anyhop started (sing-box running idle — no channels yet). "
            "Add one: anyhop channels add <provider> --country …"
        )
    print(f"Web UI: {service.web_ui_url()}  (open it: anyhop ui)")
...
def _maybe_offer_daemon_install(args) -> None:
    """The one-time login-service offer after a manual `anyhop start`.

    The manual `anyhop daemon install` path stays canonical; this only surfaces
    ...
    (`ANYHOP_SERVICE=1` — the service manager already owns it), a container
    ...
    if getattr(args, "yes", False):
        ...
        print("Login service installed (anyhop daemon install).")
        return
    store = Store.load()
    if getattr(args, "no_service", False):
        ...
        return
    if os.environ.get("ANYHOP_SERVICE") or runtime.in_container():
        return
    ...
    try:
        answer = input(
            "Start anyhop automatically at login (installs a user-level service; "
            "you can undo with `anyhop daemon uninstall`)? [y/N] "
        )
    ...
    if answer.strip().lower() in ("y", "yes"):
        service.daemon_install()
        print("Login service installed. Undo any time: anyhop daemon uninstall")
    else:
        print("Okay — not asking again. Opt in later with: anyhop daemon install")


def cmd_stop(args):
    result = service.stop()
    if result["was_running"]:
        print("Anyhop stopped (channels kept in config).")
    else:
        print("Anyhop is already stopped.")
...
def cmd_ui(args):
    ...
    if not service.ensure_web_ui():  # start the daemon + wait for a verified serve
        sys.exit(
            ...
            "program — the sign-in link is only sent to a verified anyhop server). "
            "Try: anyhop restart"
        )
    ...
    if not args.no_open and sys.stdout.isatty():
        ...
        try:
            ...
            opened = False
    if opened:
        print(f"Opening the anyhop dashboard: {service.web_ui_url()}")
    else:
        print("Open the anyhop dashboard in your browser (one-time sign-in link):")
        print(f"  {login_url}")
...
def cmd_restart(args):
    service.restart()
    print("Anyhop restarted. See: anyhop status")
...
def cmd_upgrade(args):
    ...
    if args.check:
        ...
        if args.json:
            ...
            command = (
                "anyhop upgrade --prerelease" if args.prerelease else "anyhop upgrade"
            )
            print(
                f"anyhop {result['current']} — {result['latest']} is available "
...
def cmd_health(args):
    ...
    monitoring (container HEALTHCHECK, cron, scripts) — `anyhop status` is the
...
def cmd_daemon_install(args):
    ...
    print(f"{verb} the anyhop login service ({result['manager']}).")
...
def cmd_daemon_uninstall(args):
    ...
    if result.get("removed"):
        print(f"Removed the anyhop login service ({result['manager']}).")
        print("  Your ~/.anyhop state (providers, channels, keys) is untouched.")
    else:
        print("No anyhop login service is installed.")
...
def cmd_run(args):
    ...
    terminal) shows the same timeline as `anyhop logs`."""
...
def cmd_helper_install(args):
    ...
    print(
        ...
        "  install, `anyhop tun on` needs no sudo — the helper owns sing-box\n"
...
def cmd_helper_uninstall(args):
    ...
    if result.get("removed"):
        ...
        print("  `anyhop tun on` now needs the sudo path again until reinstalled.")
    else:
...
def _helper_status_text(s: dict) -> str:
    ...
    if not s.get("installed"):
        return (
            ...
            "Install once:  sudo anyhop helper install"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ca.add_argument(
        ...
        "declare one when something outside anyhop must know it ahead of time",
    )
    ...
    cr.add_argument(
        ...
        nargs="+",
        action="append",
        help="legacy form: anyhop channels rm <provider> --channel <name>",
    )
    cr.add_argument(
        "--all",
        action="store_true",
    ...
    rsa.add_argument("ruleset", help="ruleset id shown by: anyhop routes ls")
    ...
    rsrm.add_argument("ruleset", help="ruleset id shown by: anyhop routes ls")
    rsrm.add_argument(
        "--dry-run", action="store_true", help="show what would be removed"
    )
    rsrm.set_defaults(func=cmd_routes_ruleset_rm)
    rsrn = rs_sub.add_parser("rename", help="rename a ruleset")
    rsrn.add_argument("ruleset", help="ruleset id shown by: anyhop routes ls")
    ...
    rst.add_argument("ruleset", help="ruleset id shown by: anyhop routes ls")
    rst.add_argument(
        "target",
        help="exit for matched traffic: <provider>/<channel>, 'direct', or 'block'",
    )
    ...
    rsu.add_argument("ruleset", help="ruleset id shown by: anyhop routes ls")
    ...
    rrm.add_argument("ids", nargs="+", help="rule id(s) shown by: anyhop routes ls")
    rrm.add_argument(
        "--dry-run", action="store_true", help="show what would be removed"
    )
    ...
    rmv.add_argument("ids", nargs="+", help="rule id(s) shown by: anyhop routes ls")
    rmv.add_argument(
        "ruleset", help="destination ruleset id shown by: anyhop routes ls"
    )
    ...
    sta.add_argument(
        "--yes",
        action="store_true",
        help="also install the login service (anyhop daemon install) without asking",
    )
    sta.add_argument(
        "--no-service",
        action="store_true",
    ...
    up = sub.add_parser(
        "upgrade",
        help="upgrade anyhop via the tool that installed it (Homebrew/uv tool/pipx/pip), "
    ...
    tn.add_argument(
        ...
        type=int,
        metavar="SECONDS",
        help="with 'on': auto-revert unless `anyhop tun confirm` runs within the "
    ...
    ex.add_argument(
        "--out",
        help="output file (default: anyhop-backup-<date>-<time>.yaml, written 0600; "
        "'-' for stdout)",
    )
    ...
    im.add_argument("file", help="bundle file from `anyhop export` (or hand-written)")
    im.add_argument(
        "--replace",
        action="store_true",
    ...
    gw = sub.add_parser(
        "gateway",
        help="container gateway profile (ANYHOP_GATEWAY=1): declare the "
    ...
    lg = sub.add_parser("logs", help="show anyhop's operation log")
    ...
    dm = sub.add_parser(
        "daemon", help="install/remove anyhop as a login service (advanced)"
    )
    ...
    ve = sub.add_parser("version", help="print anyhop's version")
    ve.add_argument(
        "--singbox-path",
        action="store_true",
    ...
    # tun mode, installed once via sudo so `anyhop tun on` never needs sudo again.
    hp = sub.add_parser(
        "helper",
        help="install/remove the privileged tun helper (root LaunchDaemon, "
        "macOS; `sudo anyhop helper install` once, then `anyhop tun on` needs no "
        "sudo)",
    )
    ...
    hi.add_argument(
        ...
        "serves another ANYHOP_HOME or is currently holding tun up",
    )
