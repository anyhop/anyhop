"""Per-provider login credentials, stored locally under ~/.anyhop/credentials.yaml.

anyhop authenticates to each provider with credentials the user adds explicitly
(``anyhop providers add <name>``) rather than reading them from the environment. A provider
is "configured" iff it has a complete credential entry in this file. The file is
written ``0600`` and never leaves the machine; anyhop displays only a masked
...
from anyhop import fsio, paths
from anyhop.state import StoreReadError, _quarantine, _read_store_text
...
def _save_all(providers: dict[str, dict]) -> None:
    ...
    def dump(f):
        f.write(
            "# Managed by anyhop. Provider login credentials — keep this file private.\n"
        )
        yaml.safe_dump(
...
def transaction():
    ...
    as :func:`anyhop.state.transaction`, on its own lock file.
    """
