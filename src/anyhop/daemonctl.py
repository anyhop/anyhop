"""Install/uninstall anyhop's background daemon as a user-level login service.

This is the plumbing behind ``anyhop daemon install`` — the CLI is the single home
of service-management logic (never a shell script or docs recipe, which are not
version-matched to the installed anyhop). Two backends:
...
Both **exec the stable console-script shim** ``anyhop applier`` (e.g.
``~/.local/bin/anyhop``), not a path inside the versioned venv: uv/pipx keep that
...
``anyhop applier`` also matches the daemon's PID-identity markers. The unit sets
``ANYHOP_SERVICE=1`` so the daemon knows it is supervised and carries ``ANYHOP_HOME``
...
from anyhop import applog, paths

LAUNCHD_LABEL = "io.github.anyhop.anyhop"
SYSTEMD_UNIT = "anyhop.service"
_APP_ENV_PATHS = {"ANYHOP_EXECUTABLE", "ANYHOP_SINGBOX", "ANYHOP_SERVICE_PREFIX"}
...
def _service_exec() -> list[str]:
    """The stable command a unit should exec: the ``anyhop`` shim + ``applier``.

    Prefers the console-script shim on PATH (stable across upgrades); falls back
    to ``<python> -m anyhop applier`` only when no shim is found (e.g. an odd
    install). Both spellings contain the ``anyhop applier`` PID marker.
    """
    ...
    return [sys.executable, "-m", "anyhop", "applier"]
...
def _app_env() -> dict[str, str]:
    ...
    for key in _APP_ENV_PATHS:
        ...
        if value and os.path.isabs(value):
            out[key] = value
    owner = os.environ.get("ANYHOP_SERVICE_OWNER")
    if owner and "\n" not in owner and "\r" not in owner:
        out["ANYHOP_SERVICE_OWNER"] = owner
    return out


def _service_env(command: list[str] | None = None) -> dict[str, str]:
    """Environment the unit must carry: the supervised marker, and ANYHOP_HOME
    ...
    env = {"ANYHOP_SERVICE": "1", "PATH": os.pathsep.join(entries)}
    env.update(_app_env())
    if os.environ.get("ANYHOP_HOME"):
        env["ANYHOP_HOME"] = str(paths.state_dir())
    return env
...
def _require_manager() -> _Manager:
    ...
    if runtime.in_container():
        ...
        raise DaemonCtlError(
            "anyhop is running inside a container — there is no login service to "
            "install. The container runtime owns the daemon's lifecycle: run "
            "`anyhop run` as the container's main process and use a restart "
...
def is_installed() -> bool:
    """True if a login-service unit exists for anyhop (ownership-handoff signal)."""
    m = manager()
...
def install(linger: bool = False) -> dict:
    ...
    from anyhop import singbox  # lazy: keep the CLI hot path free of the sing-box stack
...
def uninstall() -> dict:
    """Remove the login service (leaves ~/.anyhop state intact)."""
    m = _require_manager()
...
def status() -> dict:
    """Where the login service stands, for ``anyhop daemon status``."""
    m = manager()
    if m is None:
