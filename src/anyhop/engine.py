...
from anyhop.providers import ProviderError, supports_ipv6
from anyhop.state import Channel, Store, channel_fingerprint
...
# of the port is anyhop's own previous sing-box whose sockets the kernel has not
...
def _listen_addr() -> str:
    ...
    behavior): loopback. ``ANYHOP_LISTEN`` — set to ``0.0.0.0`` by the container
    ...
    value = (os.environ.get("ANYHOP_LISTEN") or "").strip()
    if not value:
        return "127.0.0.1"
    try:
        ipaddress.ip_address(value)
    except ValueError:
        applog.log(f"ANYHOP_LISTEN={value!r} is not an IP address — using 127.0.0.1")
        return "127.0.0.1"
    return value
...
def _wg_mtu() -> int:
    ...
    ``ANYHOP_WG_MTU`` overrides it for operators who know their path; an
    ...
    value = (os.environ.get("ANYHOP_WG_MTU") or "").strip()
    if not value:
        return WG_MTU
    try:
        mtu = int(value)
    except ValueError:
        applog.log(f"ANYHOP_WG_MTU={value!r} is not a number — using {WG_MTU}")
        return WG_MTU
    if not 1280 <= mtu <= 9000:
        applog.log(f"ANYHOP_WG_MTU={mtu} is outside 1280-9000 — using {WG_MTU}")
        return WG_MTU
    return mtu
...
class Engine:
    ...
    def _build_config(self) -> tuple[dict, dict[str, str]]:
        ...
        for ch in self.store.channels():
            ...
            if ch.port in self._held_ports:
                errors[f"{ch.provider}/{ch.id}"] = (
                    ...
                    "free the port (then restart anyhop) or change the "
                    "declaration"
                )
                continue
            try:
        ...
        if rule_sets:
            # Always type: local — anyhop downloaded and verified these files on
        ...
        if self.store.router.get("tun"):
            # anyhop owns resolution in TUN mode: hijacked queries are answered
    ...
    def _tun_interface_name() -> str:
        ...
        return "utun225" if sys.platform == "darwin" else "anyhop-tun"
    ...
    def _router_config(
        ...
        LAN resolver are still answered by anyhop, not leaked), the CIDR half
        ...
        if port and port in self._held_ports:
            errors["router/entrypoint"] = (
                ...
                "anyhop) or change the router: port: declaration"
            )
            ...
            if mtype in matcher_fields:
                ...
                if path is None:
                    errors[ref] = (
                        ...
                        "it is fetched: anyhop routes geo refresh"
                    )
