...
def _preserve_owner(path: str | Path, like: Path) -> None:
    ...
    A management command run as root — ``docker exec <container> anyhop …``
    (root is the exec default), a stray ``sudo anyhop …`` — must not flip a
...
def write_secret(path: Path, text: str, *, overwrite: bool = True) -> None:
    ...
    tightened). Used by ``anyhop export``.
