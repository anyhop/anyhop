...
sing-box (>= 1.12; anyhop pins 1.13) loads geo data only as *rule-sets* —
one binary ``.srs`` file per category — so anyhop consumes the upstreams that
...
``anyhop routes geo refresh``. The compiled sing-box config only ever gets
...
from anyhop import applog, paths, routes
...
def categories(
    ...
    docs — or run ``anyhop routes geo refresh``).
    """
    man = manifest()
