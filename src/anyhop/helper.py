...
(macOS has no ``setcap`` equivalent). The v1 stopgap is ``sudo … anyhop tun on``
every time; this helper is the steady state: installed **once** by
``sudo anyhop helper install`` as a system LaunchDaemon, it runs as root for the
machine's life, and the user-level anyhop delegates sing-box's lifecycle to it
...
anyhop. It does **not** parse ``state.json``, run :mod:`anyhop.engine`, or touch
credentials. The user daemon generates the config (as it always has) and writes
it to the fixed path ``$ANYHOP_HOME/singbox.json``; the helper merely ``exec``\\ s
...
installing user recorded at install time (``ANYHOP_HELPER_ALLOWED_UID``). No
...
One helper serves one machine — and exactly one ``ANYHOP_HOME``. The socket is
...
second/fresh ``ANYHOP_HOME`` on a helper-installed machine would adopt — and
could stop — the real install's root sing-box. Rebinding to another home is
one explicit step: ``sudo anyhop helper install`` from that home.
...
from anyhop import applog, paths, singbox
...
HELPER_SOCKET_DEFAULT = "/var/run/anyhop.helper.sock"
HELPER_LABEL = "io.github.anyhop.anyhop.helper"
...
# caller's ANYHOP_HOME and the helper serves exactly one home.
PROTOCOL_VERSION = 2
...
REINSTALL_HINT = "sudo anyhop helper install"
...
def _handle(req: dict, allowed_uid: int, served_home: str) -> dict:
    ...
    name the served home — state is per-``ANYHOP_HOME`` and this helper serves
    exactly one."""
    cmd = req.get("cmd")
    if cmd == "ping":
    ...
    if caller_home != served_home:
        ...
        return _err(
            f"this helper serves ANYHOP_HOME {served_home}, not "
...
def _allowed_uid_from_env() -> int:
    allowed = os.environ.get("ANYHOP_HELPER_ALLOWED_UID")
    if not allowed:
        raise HelperConfigError(
            "ANYHOP_HELPER_ALLOWED_UID is unset — refusing to start"
        )
    try:
        return int(allowed)
    except ValueError:
...
def run_daemon() -> int:
    ...
    plist (``ANYHOP_HELPER_SOCKET``, ``ANYHOP_HELPER_ALLOWED_UID``, and the usual
    ``ANYHOP_HOME`` so the anyhop path machinery points at the installing user's
    ...
    try:
        ...
        applog.log(f"anyhop-helper: {e}")
        return 2
    socket_path = os.environ.get("ANYHOP_HELPER_SOCKET", HELPER_SOCKET_DEFAULT)
    # The served home: resolved once from the install plist's ANYHOP_HOME (via
...
def _serve_forever(socket_path: str, allowed_uid: int, served_home: str) -> NoReturn:
    ...
    while True:
        try:
            ...
            applog.log(f"anyhop-helper: accept failed: {e}")
            continue
        try:
            ...
            if uid != 0 and uid != allowed_uid:
                # uid 0 (root) is always allowed so anyhop run under sudo, and a
            ...
            try:
                ...
                pass
            applog.log(f"anyhop-helper: request error: {e}")
        finally:
            try:
                conn.close()
            except OSError:
...
def _client_home() -> str:
    """This process's ANYHOP_HOME, as sent with every state command."""
...
def request(cmd: str, **fields) -> dict:
    ...
    downgrade.
    """
    path = os.environ.get("ANYHOP_HELPER_SOCKET", HELPER_SOCKET_DEFAULT)
    try:
