"""Presentation helpers for the anyhop CLI.
...
def _base_cells(c: dict) -> list[str]:
    ...
    Reads the fields every channel dict from ``anyhop.service`` carries: label,
...
def _table(headers: list[str], rows: list[list[str]]) -> list[str]:
    ...
    the ``header`` / ``------`` / ``rows`` layout shared by every anyhop table.
    """
    n = len(headers)
...
def providers_list(data: dict) -> str:
    ...
    if not providers:
        return "No providers added yet. Add one:  anyhop providers add nordvpn"
...
def channels_list(data: dict) -> str:
    ...
    if not channels:
        return 'No channels yet. Add one:  anyhop channels add nordvpn --country "United States"'
    ...
    if "country" in data:
        if not data.get("matched", True):
            return with_warning(
                ...
                f"See the full list: anyhop locations {provider}"
            )
...
def routes_list(data: dict) -> str:
    ...
    if data.get("flat") or data.get("filter"):
        if not rules:
            if data.get("filter"):
                return (
                    f"No matchers targeting {data['filter']!r}. See: anyhop routes ls"
                )
            return "\n".join(
                [
                    head,
                    ...
                    "without a VPN.",
                    "Add one:  anyhop routes ruleset create Streaming --via <provider>/<channel> --domain netflix.com",
                ]
            )
    ...
    if dns:
        if dns.get("error"):
            ...
            if dns.get("aaaa_suppressed"):
                lines.append(
                    "  note: anyhop's tun DNS suppresses these AAAA answers "
...
def test_result(data: dict) -> str:
    ...
    if not channels:
        if data.get("filter"):
            return f"No channel named {data['filter']!r}. See: anyhop channels ls"
        return (
...
def daemon_status(data: dict) -> str:
    ...
    if not svc.get("supported"):
        ...
        lines.append("  Install with: anyhop daemon install")
    ...
    if d["running"]:
        ...
        if d["skew"]:
            lines.append(
                f"  ⚠ CLI is {d['cli_version']} — run `anyhop restart` to match."
            )
...
def _skew_lines(snapshot: dict) -> list[str]:
    ...
    if d.get("skew"):
        return [
            ...
            "run `anyhop restart` to pick up the upgrade"
        ]
    return []
...
def _channels_summary(channels: list[dict]) -> str:
    ...
    from anyhop.providers import display_name
...
def status(snapshot: dict) -> str:
    ...
    posture, Web UI. Deliberately no per-channel table — `anyhop test` is the one
    ...
    if not snapshot["running"]:
        lines = ["Anyhop - Inactive", *_skew_lines(snapshot), *_runtime_lines(snapshot)]
        if channels:
            ...
            lines.append(
                f"  ({detail} across "
                f"{snapshot['provider_count']} provider(s) configured; run `anyhop start`)"
            )
        return "\n".join(lines)

    lines = ["Anyhop - Active", *_skew_lines(snapshot), *_runtime_lines(snapshot)]
    if channels:
        lines.append(
            f"  Channels  {_channels_summary(channels)}  (details: anyhop channels ls)"
        )
    router = snapshot.get("router")
    if router:
        lines.append(
            f"  Router    {_router_where(router)} — {_router_mode(router)}"
            "  (details: anyhop routes ls)"
        )
        if router.get("tun"):
            lines.append(
                "  TUN       ON — all system traffic follows the routing rules"
                "  (disable: anyhop tun off)"
            )
    if snapshot.get("web_ui"):
        lines.append(f"  Web UI    {snapshot['web_ui']}  (open it: anyhop ui)")
    ...
    if not channels:
        lines.append(
            '  (no channels yet — add one: anyhop channels add nordvpn --country "United States")'
        )
    return "\n".join(lines)
