...
def proxy_opener(port: int):
    ...
    ``port`` — the one way anyhop drives traffic through a channel (shared with
    the speed test in :mod:`anyhop.throughput`)."""
...
# categories) so every surface — `anyhop test`, `anyhop status`, the Web UI —
