...
anyhop addresses its detached processes (sing-box, the applier daemon) by
...
anyhop as running, block a real start, and (worst) let ``stop()`` escalate to
...
start time, so matching it proves the process is the very one anyhop spawned,
...
def record(pid: int) -> dict:
    """The identity record to persist for a process anyhop just spawned.
...
def verify(rec: dict, markers: tuple[str, ...]) -> bool:
    """True iff the recorded process is alive and is the one anyhop spawned.
