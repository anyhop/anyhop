...
``reconnect`` dict, so it survives a daemon restart:
    ...
    error     why we gave up (shown by ``anyhop status``)
...
human intervenes (``anyhop restart`` clears every flag; re-adding a channel starts
it fresh).
...
def _killswitch_diagnostic(store: Store) -> str:
    """A targeted note when anyhop's own recovery egress is policy-blocked.

    With TUN + kill switch on and the tunnel down, anyhop's provider-API calls
...
def run_pass(
    ...
    for ch in store.channels():
        ...
        if rc.get("failed"):
            continue  # already gave up; wait for a human (anyhop restart / re-add)
