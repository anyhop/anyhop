...
def _blockers_error(
    ...
    lines.append(f"Remove the rules first:  anyhop routes rm {' '.join(ids)}")
    err = ServiceError("\n".join(lines))
...
def provider_update_token(provider: str, creds: dict) -> dict:
    ...
    if not store.has_provider(provider):
        raise ServiceError(
            f"{display_name(provider)} is not added — run `anyhop providers add {provider}` first."
        )
...
def channel_add(
    ...
    if not country:
        raise ServiceError(
            f"usage: anyhop channels add {provider} --country <country> [--city <city>]"
        )
...
def _channel_public(channel, revision: str | None = None) -> dict:
    ...
    key), so the raw :class:`~anyhop.state.Channel` dataclass must never be
...
def _resolve_channel_ref(store: Store, ref: str, provider: str | None) -> list[dict]:
    ...
    if not matches:
        raise ServiceError(f"no channel named {ref!r} (see: anyhop channels ls).")
    if not _is_pattern(ref) and len(matches) > 1:
        ...
        raise ServiceError(
            ...
            f"use a qualified ref like: anyhop channels rm {matches[0]['ref']}"
        )
    return matches
        ...
        for item in plan:
            ...
            if kind(item["provider"]) != "token" or not is_functional(item["provider"]):
                raise ServiceError(
                    ...
                    f"Re-import its .conf: anyhop channels add {item['provider']} "
                    "--config <file>"
                )
            try:
...
def routes_ruleset_create(name: str, target: str, matchers: list) -> dict:
    ...
    try:
        ...
        raise ServiceError(f"{e} (see: anyhop channels ls --refs).") from e
...
def routes_ruleset_remove(ruleset_id: str, dry_run: bool = False) -> dict:
    ...
    if block is None:
        raise ServiceError(f"unknown ruleset {ruleset_id!r} (see: anyhop routes ls).")
    shadows = routes.shadowed_by(store.rules())
...
def routes_remove(ids: list[str], dry_run: bool = False) -> dict:
    if not ids:
        raise ServiceError("at least one rule id is required (see: anyhop routes ls).")
    ids = list(dict.fromkeys(ids))
    store = Store.load()
    existing = {rule["id"]: rule for rule in store.rules()}
    missing = [i for i in ids if i not in existing]
    if missing:  # all misses in one pass, like the removal blockers
        raise ServiceError(f"no rule(s) {', '.join(missing)} (see: anyhop routes ls).")
...
def routes_move(ids: list[str], ruleset_id: str) -> dict:
    ...
    try:
        ...
        raise ServiceError(f"{e} (see: anyhop routes ls).") from e
    shadows = routes.shadowed_by(store.rules())
    decorated = _decorate_ruleset(block, shadows)
    wanted = set(ids)
...
def routes_reorder(
    ...
    if not ids:
        ...
        raise ServiceError(
            f"at least one {what} id is required (see: anyhop routes ls)."
        )
    store = Store.load()
    try:
        if flat:
...
def routes_lan_direct(enable: bool | None = None) -> dict:
    ...
    The rules themselves (:data:`anyhop.routes.LAN_DIRECT_CIDRS` plus the
    :data:`anyhop.routes.LAN_DIRECT_UDP_PORTS` unicast-port half) are fixed and
...
def routes_trace(destination: str) -> dict:
    ...
    destination. See :mod:`anyhop.tracer` for the semantics and their limits.
    """
    try:
...
_TUN_HELPER_HINT = (
    "install it once, then `anyhop tun on` needs no sudo:\n  sudo anyhop helper install"
)
...
_TUN_SUDO_HINT = (
    'one-off: anyhop stop && sudo ANYHOP_HOME="$HOME/.anyhop" anyhop tun on'
)
...
def _require_tun_privileges() -> None:
    ...
    - **privileged helper installed** (macOS) — the root LaunchDaemon owns
      ...
      this is the steady state after `sudo anyhop helper install`;
    ...
    A helper that answers but serves a *different* ``ANYHOP_HOME`` (or predates
    ...
    if hp["state"] == "foreign":
        raise ServiceError(
            f"the privileged helper serves a different ANYHOP_HOME ({hp['home']}); "
    ...
    with fsio.locked(_tun_trial_lock()):
        ...
        if data is None:
            ...
            applog.log(
                ...
                "reverting TUN mode off (re-enable with: anyhop tun on)"
            )
        ...
        if remaining <= 0:
            _tun_trial_path().unlink(missing_ok=True)
            applog.log(
                "TUN trial expired while anyhop was not running: reverting TUN mode off"
            )
            if Store.load().router.get("tun"):
                _set_tun(False)
...
# red until the data plane actually holds. Opt-in via ANYHOP_GATEWAY=1 — a host
# install never sees any of this.


def gateway_profile_active() -> bool:
    """The container gateway profile is declared via ``ANYHOP_GATEWAY=1``."""
    import os

    return os.environ.get("ANYHOP_GATEWAY") == "1"
...
def _gateway_privilege_problems() -> list[str]:
    ...
    if os.geteuid() != 0:
        problems.append(
            ...
            'ANYHOP_RUN_AS_ROOT=1 (and user: "0" if the image user was overridden)'
        )
...
def gateway_init() -> dict:
    ...
    ``ANYHOP_GATEWAY=1``. Privilege-checks first — a gateway container missing
...
def setup_sync(text: str) -> dict:
    ...
    disable`` survive. See ``anyhop sync`` / docs/docker.md.
    """
    lookup, notes = _bundle_location_lookup()
    try:
        _ensure_geo_from_bundle(text)
...
def web_ui_login_url() -> str:
    """A one-time login URL for opening the Web UI (used by ``anyhop ui``)."""
    from anyhop.api import server as api_server
...
def _daemon_info() -> dict:
    """The applier daemon's status + CLI↔daemon version skew for ``anyhop status``."""
    info = daemon.daemon_info()
...
def _test_row(channel, probe: dict, traffic: dict) -> dict:
    ...
    return {
        ...
        "probe": probe,
        # Cumulative traffic totals ride along on every test row: `anyhop test`
        # is the one per-channel table, so the durable counters (see
        # anyhop.metrics) surface here rather than in a separate command.
...
def metrics_totals(channel: str | None = None) -> dict:
    ...
    ``anyhop.metrics``); channels with no recorded traffic report zeros.
    """
    store = Store.load()
    channels = store.channels()
    if channel is not None:
...
def daemon_install(linger: bool = False) -> dict:
    """Register the daemon as a user-level login service (see anyhop.daemonctl).
...
def daemon_uninstall() -> dict:
    ...
    was successfully removed. State under ``~/.anyhop`` is left intact.
    """
    try:
...
def daemon_status() -> dict:
    """Login-service + running-daemon status for ``anyhop daemon status``."""
    ...
    if result["changed"] and (daemon.in_daemon_process() or daemon.is_running()):
        if result["channel"] == "homebrew":
            ...
            if info.get("service_owner") == "homebrew":
                ...
                result["restart_command"] = "brew services restart anyhop"
        else:
            # From the Web UI this schedules the restart after the response is
            # flushed; from the CLI it restarts through anyhop's login service or
...
def helper_install(takeover: bool = False) -> dict:
    """Install the root tun helper (``sudo anyhop helper install``).

    The thin service wrapper: the privilege/root checks live in
    :mod:`anyhop.helperctl` so they are testable without going through the CLI.
    ``takeover`` forces a rebind of the machine's single helper away from
    another install's ``ANYHOP_HOME`` (and past a live tun).
    """
    from anyhop import helperctl
...
def helper_status() -> dict:
    ...
    # `anyhop helper status` distinguishes "installed but crashed" from "live".
    if s.get("installed"):
