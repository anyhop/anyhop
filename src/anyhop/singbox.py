...
anyhop runs exactly one sing-box instance. Each enabled channel becomes a local
...
``~/.anyhop/bin/sing-box@<version>`` and checksum-verified. Any other sing-box on
...
from anyhop import applog, fsio, paths, proc
...
class ConfigRejectedError(SingBoxError):
    ...
    a timer cannot help — only a state change (or an anyhop upgrade) can."""
...
def host_platform() -> str:
    ...
    if key not in SINGBOX_SHA256:
        ...
        raise SingBoxError(
            f"unsupported platform {system}/{machine}. anyhop ships sing-box for: {supported}."
        )
    return key
...
def bin_path() -> Path:
    ...
    """
    override = os.environ.get("ANYHOP_SINGBOX")
    if override:
...
def _download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "anyhop/1"})  # noqa: S310
...
def ensure_binary() -> Path:
    ...
    target = bin_path()
    if os.environ.get("ANYHOP_SINGBOX"):
        ...
        if not target.exists():
            raise SingBoxError(f"ANYHOP_SINGBOX={target} does not exist")
        got = _sha256_file(target)
        if got != expected:
            raise SingBoxError(
                f"ANYHOP_SINGBOX={target} is not the pinned sing-box "
    ...
    with fsio.locked(target.with_name(target.name + ".lock")):
        ...
        if target.exists():
            print(
                f"anyhop: sing-box at {target} failed checksum verification — "
...
def _repair_cached(target: Path) -> None:
    ...
    changed out from under anyhop (chmod'd non-executable, or made group/world
    ...
    if uid != os.geteuid() and os.geteuid() != 0:
        raise SingBoxError(
            ...
            "user — remove it and rerun so anyhop re-downloads its own copy."
        )
...
def _install(key: str, expected: str, target: Path) -> Path:
    ...
    print(
        f"anyhop: downloading sing-box {SINGBOX_VERSION} ({key}) from {url}\n"
        ...
        with tarfile.open(archive) as tf:
            ...
            try:
                ...
                if os.path.exists(staged):
                    os.unlink(staged)
    print(f"anyhop: verified sing-box {SINGBOX_VERSION} ({key}).", file=sys.stderr)
...
def _prune_old_binaries(target: Path) -> None:
    ...
    try:
        ...
        for stale in old[1:]:  # keep the newest previous version as rollback
            ...
            print(
                f"anyhop: pruned old sing-box cache entry {stale.name}", file=sys.stderr
            )
    except OSError:
        pass
...
def clash_api() -> dict:
    ...
    two ``ANYHOP_HOME``\\ s) on one machine don't fight over the same port.
...
def pid_path_for(home: str | Path) -> Path:
    """The sing-box pidfile under an arbitrary ``ANYHOP_HOME``.

    Split out of :func:`_pid_path` so a privileged caller can inspect *another*
    home's sing-box without adopting that home — :mod:`anyhop.helperctl` needs it
...
class Runner:
    ...
    def _helper_identity(self) -> _Identity:
        ...
        second ``ANYHOP_HOME`` would claim, and could stop, another install's
        root sing-box.
        """
        if self.local_only:
            return _NOTHING_RUNNING
        from anyhop import helper, paths
    ...
    def apply(self, config: dict) -> ApplyResult:
        ...
        not edited out from under the running process by accident — anyhop
        ...
        daemon held alive by the Clash API controller), so ``anyhop start`` with
        zero channels still reports an active daemon. It is only stopped
        explicitly by ``anyhop stop``.
        """
        ...
        if has_tun and hp["state"] in ("stale", "foreign"):
            ...
            if hp["state"] == "foreign":
                detail = (
                    f"the privileged helper serves a different ANYHOP_HOME "
    ...
    def check(self, binary: Path | None = None) -> bool:
        ...
        if returncode != 0:
            ...
            print(
                f"anyhop: sing-box rejected the config:\n{error}",
                file=sys.stderr,
            )
            return False
        self._check_error = ""
    ...
    def logs(self, tail: int = 80) -> str:
        from anyhop.applog import reverse_tail
