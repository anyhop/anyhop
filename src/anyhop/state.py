"""The single consolidated state file: ``~/.anyhop/state.json``.

This is anyhop's one source of truth for the data model: **each provider holds a
...
def _next_free_port(data: dict) -> int:
    ...
    (``ANYHOP_PORT_BASE=<n>``): allocate sequentially from ``n`` upward instead —
    ...
    base = os.environ.get("ANYHOP_PORT_BASE")
    if base:
        try:
            start = int(base)
        except ValueError as e:
            raise RuntimeError(
                f"ANYHOP_PORT_BASE must be a port number: {base!r}"
            ) from e
        if not 0 < start <= 65535:
            raise RuntimeError(f"ANYHOP_PORT_BASE must be 1-65535, got {start}")
        for port in range(start, 65536):
            if port not in used and _os_port_free(port):
                return port
        raise RuntimeError(f"no free port at or above ANYHOP_PORT_BASE={start}")
    for _ in range(100):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.bind(("127.0.0.1", 0))
...
def _next_id(taken: set, country: str, city: str, id_hint: str | None = None) -> str:
    ...
    [<city>_]<n>`` — ``wg`` (the only protocol anyhop speaks), the lowercase
    ...
    if cid:
        ...
        if not chans[cid].get("enabled", True):
            raise ValueError(
                ...
                f"anyhop channels enable {target}"
            )
...
def _normalize_rulesets(data: dict) -> None:
    ...
    as ``domain_suffix`` — anyhop has one domain semantic (the domain and its
...
class StateVersionError(StoreReadError):
    """state.json was written by a newer anyhop than this one.

    Not corruption — the data is presumably fine, this anyhop just cannot be
    trusted to interpret (let alone read-modify-write) it. Never quarantined;
    reads and mutations abort until anyhop is upgraded.
    """
...
def _quarantine(
    ...
    *,
    ...
    with held:
        ...
        applog.log(msg)
        print(f"anyhop: {msg}", file=sys.stderr)
        return True
...
def _read_raw(*, lock_held: bool = False) -> dict:
    ...
    if version > STATE_VERSION:
        raise StateVersionError(
            f"{p.name} is version {version}, newer than this anyhop understands "
            f"(max {STATE_VERSION}) — upgrade anyhop"
        )
...
def transaction():
    ...
    with fsio.locked(_lock_path()):
        ...
        setup_id = None
        txn_module = sys.modules.get("anyhop.txn")
...
class Store:
    ...
    def service_offer_recorded(self) -> bool:
        """Was the one-time `anyhop start` daemon-install offer already made?"""
        return bool(self.data.get("service_offered"))

    def record_service_offer(self) -> None:
        """Remember that the offer was made (accepted or declined) — `anyhop
    ...
    def clear_reconnect_all(self) -> int:
        """Drop reconnect bookkeeping from every channel (e.g. on ``anyhop restart``),
