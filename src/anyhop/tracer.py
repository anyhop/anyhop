...
:func:`anyhop.routes.match_destination`. geosite/geoip categories are answered
from the same cached, digest-verified ``.srs`` files the engine hands
sing-box, parsed by :mod:`anyhop.srs` — same data, same semantics, no running
sing-box required.

This is an offline evaluation, not a probe: nothing is sent through any
tunnel (``anyhop test`` does that). The only network I/O is the disclosed DNS
...
see :mod:`anyhop.probe`) never colors the answer. Tracing a literal IP skips
DNS entirely.
...
- The trace models the **IP-dialed** path (tun mode: apps resolve via the
  ...
  for that path.
- A destination is traced as one flow. Its family follows anyhop's DNS
...
from anyhop import geodata, routes, srs
from anyhop.constants import TUN_DNS_UPSTREAM
from anyhop.engine import channel_ipv6
            ...
            if problem:
                ...
                return finish(
                    ...
                    _segs(
                        ...
                        "until it is fixed: anyhop routes geo refresh",
                    ),
                )
