"""Upgrade anyhop via the tool that installed it — never by replacing itself.

The never-self-update invariant: anyhop's files are owned by whatever installed
them (Homebrew, uv tool, pipx, pip), and only that owner may replace them.
``anyhop upgrade`` detects the owning channel, checks its authoritative release
...
(``anyhop upgrade --check`` / the Web UI button): it asks the owning channel for
...
HOMEBREW_FORMULA_URL = (
    "https://raw.githubusercontent.com/anyhop/homebrew-tap/main/Formula/anyhop.rb"
)
...
def _pipx_receipt_owns(prefix: Path) -> bool:
    """Whether ``prefix`` is pipx's environment for anyhop."""
    try:
...
def _owning_shim_dir(channel: str) -> Path | None:
    ...
    if candidate is None or not matches:
        ...
        raise UpgradeError(
            f"the detected {channel} environment owns this anyhop installation, "
            f"but PATH resolves the anyhop shim to {rendered} instead of {expected}; "
...
def detect_channel() -> str:
    ...
    if "/cellar/anyhop/" in low or "/homebrew/opt/anyhop/" in low:
        return "homebrew"
...
_REFUSALS = {
    "container": (
        "this anyhop runs inside a container image — the image is immutable. "
        "Upgrade by pulling a new image tag and recreating the container."
    ),
    "checkout": (
        "this anyhop is a git checkout — upgrade it with git (git pull) and "
        "your usual environment sync, not a package manager."
    ),
    "unknown": (
        "could not determine the install channel; upgrade anyhop with the tool "
        "that installed it."
    ),
}
    ...
    try:
        try:
            ...
            raise UpgradeBusyError(
                "another anyhop upgrade is already running; wait for it to finish "
                "and try again."
            ) from e
        except OSError as e:
            raise UpgradeError(
...
def _upgrade_lock_directory() -> Path:
    ...
    ``ANYHOP_HOME`` selects runtime/config state, but it does not select a second
...
def _homebrew_prefix(brew: str) -> Path:
    """Return this brew executable's active absolute prefix for anyhop."""
    prefix_cmd = [brew, "--prefix", "anyhop"]
    try:
...
def _validate_homebrew_owner(brew: str) -> None:
    """Require PATH's brew to own the Homebrew environment running anyhop."""
    ...
    if not matches:
        raise UpgradeError(
            f"{brew} resolves anyhop to {expected}, which does not own the running "
...
def _command_for(channel: str, *, latest: str, exact_target: bool) -> list[str]:
    ...
    # The brew keg is named for the formula (`anyhop`), not the PyPI package.
    tool = {"uv-tool": "uv", "pipx": "pipx", "homebrew": "brew"}[channel]
    exe = shutil.which(tool)
    if not exe:
        raise UpgradeError(
            f"this anyhop was installed with {tool}, but `{tool}` is not on PATH — "
            f"upgrade needs the owning tool."
        )
    if channel == "uv-tool":
        # `uv tool upgrade` preserves the requirement used at install time.
        # The release bootstrap deliberately installs `anyhop==<version>`,
    ...
    if channel == "homebrew":
        ...
        return [exe, "upgrade", "anyhop"]
    # `pipx upgrade` deliberately reuses the receipt's package_or_url. That may
    # be a Git/local/direct source even though anyhop just checked PyPI. Reinstall
...
def _homebrew_installed_version(brew: str) -> str:
    ...
    version_cmd = [str(prefix / "bin" / "anyhop"), "version"]
    try:
...
def _verify_postcondition(
    *, before: str, after: str, latest: str, exact_target: bool, cmd: list[str]
    ...
    if after_v <= before_v:
        raise UpgradeError(
            f"`{rendered}` completed successfully, but anyhop did not advance "
            f"from {before} (found {after}; expected {latest})."
        )
    if exact_target and after_v != latest_v:
        raise UpgradeError(
            f"`{rendered}` completed successfully, but installed anyhop {after} "
    ...
    if not exact_target and (after_v.is_prerelease or after_v.is_devrelease):
        raise UpgradeError(
            ...
            f"anyhop {after} after a stable-release check (expected at least {latest})."
        )
...
def run(*, prerelease: bool = False) -> dict:
    ...
    with _upgrade_lock():
        ...
        if prerelease and channel == "homebrew":
            raise UpgradeError(
                "Homebrew tracks stable anyhop releases only; use a uv, pipx, or pip "
...
def _fetch_homebrew_version(timeout: float) -> str:
    ...
    This is a network read of the same formula source `brew upgrade anyhop`
    ...
    match = re.search(r"anyhop-(\d[0-9A-Za-z.+-]*)\.tar\.gz", formula)
    if not match:
        raise UpgradeError("the Homebrew tap formula carried no anyhop version")
